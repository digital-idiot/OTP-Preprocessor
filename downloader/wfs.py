...
#             dst_layer: Optional[str] = "Layer",
#             dst_mode: Optional[str] = 'w',
#             typename: Optional[Tuple[str]] = None,
#             property_name: Optional[Tuple[str]] = "*",
#             max_features: Optional[int] = None,
...
#             sort_by: Optional[Tuple[str]] = None,
#     ):
#         roi = gpd.read_file(filename=roi_file)
#         count = roi.shape[0]
#         crs = roi.crs
#
#         with Progress() as progress:
#             main_task = progress.add_task(
#                 description="[green]Total:", total=count
#             )
#             for index, row in roi.iterrows():
#                 # spatial_query = f'<ogc:Filter ' \
#                 #     f'xmlns:ogc="http://www.opengis.net/ogc" ' \
#                 #     f'xmlns:gml="http://www.opengis.net/gml">' \
#                 #     f'<ogc:Within>' \
#                 #     f'<ogc:PropertyName>geom</ogc:PropertyName>' \
#                 #     f'<gml:Polygon srsName="{crs}">' \
#                 #     f'{row.geometry.wkt}' \
#                 #     f'</gml:Polygon>' \
#                 #     f'</ogc:Within>' \
#                 #     f'</ogc:Filter>'
...
#                 )
#                 progress.update(task_id=main_task, advance=1)
